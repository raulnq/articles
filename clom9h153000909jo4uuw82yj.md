...
The [Traces API](https://github.com/dotnet/dotnet-monitor/blob/main/documentation/api/trace.md#was-this-documentation-helpful-share-feedback) enables collecting `.nettrace` formatted traces. To capture the traces of a process using a predefined set of trace profiles (`Cpu`, `Http`, `Logs`, `Metrics`)**,** navigate to `https://localhost:52323/logs?pid={pid}&durationSeconds=60` and wait to get the file. Open the `Program.cs` file and update the content as follows:
